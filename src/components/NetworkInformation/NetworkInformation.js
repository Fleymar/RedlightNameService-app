...
function NetworkInformation() {
  ...
  const [networkName, setNetworkName] = React.useState('Loading')
  const {
  ...
  getNetworkId().then(networkId => {
    if (networkId === 8457) {
      setNetworkName('Redlight Testnet')
    } else {
      ...
      {!isReadOnly ? (
        <AccountContainer>
          ...
          getReverseRecord.avatar ? (
            <Avatar
              src={imageUrl(getReverseRecord.avatar, displayName, networkName)}
            />
          ) : (
