...
import {
  ...
  REDLIGHTTESTNET_DNSREGISTRAR_ADDRESS
} from '../../utils/utils'
...
async function getRegistrarEntry(name) {
  ...
  if (nameArray.length > 3 || nameArray[1] !== 'redlc') {
    return {}
  }
...
async function setDNSSECTldOwner(ens, tld, networkId) {
  ...
  switch (networkId) {
    ...
    case 8457:
      return REDLIGHTTESTNET_DNSREGISTRAR_ADDRESS
    default:
      return emptyAddress
  }
}
...
async function getDNSEntryDetails(name) {
  ...
  if (nameArray.length !== 2 || nameArray[1] === 'redlc') return {}
...
function adjustForShortNames(node) {
  ...
  if (nameArray.length > 2 || parent !== 'redlc' || label.length > 6)
    return node
...
const resolvers = {
  Query: {
    publicResolver: async () => {
      try {
        const ens = getENS()
        const resolver = await ens.getAddress('resolver.redlc')
        return {
          address: resolver,
          __typename: 'Resolver'
        }
    ...
    getResolverMigrationInfo: async (_, { name, resolver }) => {
      ...
      async function calculateIsPublicResolverReady() {
        const publicResolver = await ens.getAddress('resolver.redlc')
