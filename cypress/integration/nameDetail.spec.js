...
describe('Name detail view', () => {
  it('should redirect if searchTerm and normalized name does not match', () => {
    cy.visit(`${NAME_ROOT}/%E2%80%8Btest.redlc`)
    cy.wait(10000)
    cy.url().should('match', /\/test\.redlc\/?/)
  })
  it('can see list of top level domains from [root]', () => {
    cy.visit(`${NAME_ROOT}/[root]/subdomains`)
    cy.queryByTestId('redlc', { timeout: 30000 }).should('exist')
    cy.queryByTestId('reverse', { timeout: 1000 }).should('exist')
    cy.url().should('eq', `${NAME_ROOT}/[root]/subdomains`)
  })
  it('cannot transfer ownership to a non-redlight address', () => {
    cy.visit(`${NAME_ROOT}/awesome.redlc`)
    cy.wait(10000)
  ...
  it('can change the resolver to the public resolver', () => {
    cy.visit(`${NAME_ROOT}/superawesome.redlc`)
  ...
  it(`prevents user from adding a record that isn't an address`, () => {
    cy.visit(`${NAME_ROOT}/notsoawesome.redlc`)
    ...
    cy.getByTestId('name-details').within(container => {
      ...
      cy.getByPlaceholderText('Enter a RLF Address', {
        timeout: 10000,
        exact: false
  ...
  it('can add record', () => {
    ...
    const text = 'Hello'
    const url = `${NAME_ROOT}/notsoawesome.redlc`
    cy.visit(url)
    ...
    cy.getByTestId('name-details').within(container => {
      ...
      cy.getByText('Addresses')
        ...
        .getByPlaceholderText('Enter a RLF Address', {
          timeout: 10000,
          exact: false
        })
        .type(address, {
  ...
  it('can change the record', () => {
    const url = `${NAME_ROOT}/abittooawesome.redlc`
    cy.visit(url)
  ...
  it('cannot change deprecated ipns contenthash', () => {
    const url = `${NAME_ROOT}/abittooawesome2.redlc`
    cy.visit(url)
  ...
  it('can navigate to a subdomain', () => {
    cy.visit(`${NAME_ROOT}/subdomaindummy.redlc/subdomains`, {
      timeout: 10000
    })
    cy.getByText('original.subdomaindummy.redlc', { timeout: 15000 }).click({
      force: true
    })
  })

  it('can add a subdomain', () => {
    const LABEL = 'sub1' // using the same subdomain label which is used at sub1.testing.redlc
    cy.visit(`${NAME_ROOT}/subdomaindummy.redlc/subdomains`, {
      timeout: 10000
    })
    cy.wait(5000)
    ...
    cy.queryByText(`${LABEL}.subdomaindummy.redlc`, {
      timeout: 10000
    }).should('exist')
  })

  it('can view records that the user does now own', () => {
    cy.visit(`${NAME_ROOT}/otherowner.redlc`, { timeout: 10000 })
