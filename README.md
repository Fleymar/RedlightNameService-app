...
As work has begun on the next version of the RNS app, please do not submit PRs for
...
The RNS app can be used with the Gnosis Safe web interface. The required steps are outline [here](./docs/gnosis_safe_app_support.md).
...
The main package for the E2E tests is `ensdomains/mock`, which exposes a script that will prepopulate ganache with RNS so you have everything setup to run Cypress on.

The RNS app has end to end tests with Cypress. To run them you need to start ganache, run the seed script, run the app and then run cypress. This should start chrome and the Cypress GUI. Each time the test run, the script needs to be re-run and the app restarted for it to work.
...
### Deploy RNS contracts and update subgraph.yml
...
`subgraph` job updates RNS contract addresses and updates environment from `mainnet` to `dev`
